...
public class OrderTransformationService {
    ...
    public OrderJson transform(String base64Document) {
        byte[] decodedXmlBytes;

        try {
            decodedXmlBytes = Base64.getDecoder().decode(base64Document);
        } catch (IllegalArgumentException e) {
            throw new InvalidBase64Exception("Document is not a valid Base64 string.");
        }

        try {
            OrderXml orderXml = xmlMapper.readValue(decodedXmlBytes, OrderXml.class);
            return orderMapper.toOrderJson(orderXml);
        } catch (Exception e) {
            throw new InvalidXmlException("Message could not be parsed from XML to JSON.", e);
        }
    }
}